...
class Cartflows_Pro_Gateway_Paypal_Express extends Cartflows_Pro_Paypal_Gateway_helper {
	...
	public function store_offer_transaction( $order, $response, $product ) {
		...
		if ( ! isset( $response['PAYMENTINFO_0_TRANSACTIONID'] ) ) {
			...
			$reason_code = $response['PAYMENTINFO_0_REASONCODE'];
		}

		if ( wcf_pro()->utils->is_separate_offer_order() ) {
			// Prepre transaction data to store in Order object requried for refund .
			$txn_data['refundable_txns'][] = array(
			...
			$order->update_meta_data( 'cartflows_offer_txn_data_' . $product['step_id'], $txn_data );
		}
	...
	public function add_subscription_payment_meta_for_ppec( $subscription, $order, $offer_product ) {
		...
		if ( 'ppec_paypal' === $order->get_payment_method() ) {
			...
			$subscription->update_meta_data( '_ppec_billing_agreement_id', $order->get_meta( '_ppec_billing_agreement_id', true ) );
			$subscription->save();
		}
	}
	...
	public function store_required_meta_keys_for_refund( $parent_order, $child_order, $transaction_id ) {
		...
		if ( ! empty( $transaction_id ) ) {
			...
			$step_id  = $child_order->get_meta( '_cartflows_offer_step_id' );
			$txn_data = $parent_order->get_meta( 'cartflows_offer_txn_data_' . $step_id );

			if ( ! empty( $txn_data ) ) {
				$child_order->update_meta_data( '_woo_pp_txnData', $txn_data );
			}
