...
class BWFAN_unsubscribe {
	...
	protected function set_data() {
		...
		$recipient = empty( $recipient ) ? filter_input( INPUT_GET, 'recipient' ) : $recipient;
		...
		/** getting recipient in case uid present */
		if ( ! empty( $uid ) ) {
		...
		/** If contact object not set and recipient available */
		if ( empty( $this->contact ) && ! empty( $recipient ) ) {
			if ( class_exists( 'BWFCRM_Common' ) ) {
				$crm_contact = BWFCRM_Common::get_contact_by_email_or_phone( $recipient );
				if ( $crm_contact instanceof BWFCRM_Contact && $crm_contact->is_contact_exists() ) {
					$this->crm_contact = $crm_contact;
					$this->contact     = $crm_contact->contact;
				}
			} elseif ( is_email( $recipient ) ) {
				$contact = new WooFunnels_Contact( '', $recipient );
				if ( $contact instanceof WooFunnels_Contact && $contact->get_id() > 0 ) {
					$this->contact   = $contact;
					$this->recipient = $contact->get_email();
				}
		...
		$lists = ! empty( $lists ) ? stripslashes_deep( $lists ) : [];
