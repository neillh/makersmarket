	...
	class VI_WOOCOMMERCE_ORDERS_TRACKING_ADMIN_API_V1 {
		...
		public function set_tracking_numbers( $request ) {
			...
			for ( $i = 0; $i < count( $tracking_data ); $i ++ ) {
				...
				$result = VI_WOOCOMMERCE_ORDERS_TRACKING_DATA::set_tracking( $order_id, $tracking_number, $carrier_slug, $item_id, $quantity_index, false, false, false, $send_sms );
								...
								for ( $i = 0; $i < count( $send_paypal ); $i ++ ) {
									...
									foreach ( $add_paypal['errors'] as $error ) {
										if ( is_array( $error['details'] ) && count( $error['details'] ) ) {
											if ( ! empty( $error['details'][0]['value'] ) && ( $send_paypal_item['tracking_number'] == $error['details'][0]['value'] || $send_paypal_item['trans_id'] === $error['details'][0]['value'] ) ) {
