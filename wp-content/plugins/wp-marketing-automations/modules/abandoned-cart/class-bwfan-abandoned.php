...
class BWFAN_Abandoned_Cart {
	...
	public function remove_abandoned_data_from_table( $order_id ) {
		...
		$order_meta_automation = $order->get_meta( '_bwfan_ab_cart_recovered_a_id', true );
		if ( ! empty( $order_meta_automation ) ) {
			$order_recovered_id = $order->get_meta( '_bwfan_recovered_ab_id', true );
			if ( empty( $order_recovered_id ) ) {
				/** save abandoned id in order meta */
				$order->update_meta_data( '_bwfan_recovered_ab_id', $cart_details['ID'] );
				$order->save_meta_data();
	...
	public function maybe_attribute_cart_recovery( $order_id, $order, $email ) {
		...
		$result = $result[0];
		$order->update_meta_data( '_bwfan_ab_cart_recovered_a_id', $result['automation_id'] );
		$order->update_meta_data( '_bwfan_ab_cart_recovered_t_id', $result['track_id'] );
		$order->update_meta_data( '_bwfan_recovered_ab_id', $cart_details['ID'] );
		$order->save_meta_data();
