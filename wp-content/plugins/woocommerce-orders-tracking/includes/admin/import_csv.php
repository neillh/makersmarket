...
class VI_WOOCOMMERCE_ORDERS_TRACKING_ADMIN_IMPORT_CSV {
	...
	public function add_menu() {
		add_submenu_page(
			'woocommerce-orders-tracking', esc_html__( 'Import Tracking', 'woocommerce-orders-tracking' ), esc_html__( 'Import Tracking', 'woocommerce-orders-tracking' ), VI_WOOCOMMERCE_ORDERS_TRACKING_DATA::get_required_capability( 'import' ), 'woo-orders-tracking-import-csv', array(
				$this,
				'import_csv_callback'
			)
		);
	...
	private static function error_log( $api_error, $service_carrier_type, $tracking_number, $order_id ) {
		if ( $api_error ) {
			...
			if ( $order_id ) {
				$log = sprintf( esc_html__( "Error adding tracking number %s to %s for order #%s: %s", 'woocommerce-orders-tracking' ), $tracking_number, $service_carrier_name, $order_id, $api_error );
			} else {
					...
					if ( $map_order_item > - 1 ) {
						...
						foreach ( $data as $item ) {
							...
							if ( is_array( $carrier ) && count( $carrier ) ) {
								...
								if ( $item_id ) {
									$result = VI_WOOCOMMERCE_ORDERS_TRACKING_DATA::set_tracking( $order_id, $tracking_number, $carrier_slug, $item_id, $quantity_index, false, false, false, false );
									...
									if ( ! $result['error'] ) {
										...
										self::wc_log( esc_html__( "Order #{$order_id}, item {$item_id}: {$result['error']}", 'woocommerce-orders-tracking' ), WC_Log_Levels::NOTICE );
									}
                        ...
                        <form class="<?php echo esc_attr( self::set( 'import-container-form' ) ) ?> vi-ui form"
                            ...
                            <div class="vi-ui segment">
                                <table class="form-table">
                                    ...
                                    <tr>
                                        <th>
                                            <label for="<?php echo esc_attr( self::set( 'custom_start' ) ) ?>"><?php esc_html_e( 'Start at row', 'woocommerce-orders-tracking' ); ?></label>
                                        </th>
                                        <td>
                                            <input type="number"
                                            ...
                                            <select name="<?php echo esc_attr( self::set( 'order_status', true ) ) ?>"
												...
												if ( count( $all_order_statuses ) ) {
													foreach ( $all_order_statuses as $status_id => $status_name ) {
														?>
                                                        <option value="<?php echo esc_attr( $status_id ) ?>" <?php selected( self::$settings->get_params( 'order_status' ), $status_id ) ?> ><?php echo esc_html( $status_name ) ?></option>
														<?php
													}
												}
												?>
                            ...
                            <div class="vi-ui segment">
                                <table class="form-table">
									...
									$description     = array(
										...
										'carrier_slug'    => '',
										'order_item_id'   => array(
											esc_html__( 'If Order Item ID is NOT mapped, the first found tracking number of an order will be set for every product line item of that order if "Manage tracking" is "Order items only" and will be set for the order otherwise.', 'woocommerce-orders-tracking' ),
											esc_html__( 'If Order Item ID is mapped but EMPTY, the respective tracking number will be set for the order if "Manage tracking" is NOT "Order items only" and will be skipped otherwise.', 'woocommerce-orders-tracking' ),
										),
										'order_status'    => esc_html__( 'If Order Status is mapped, it will override the "Change order status" option above', 'woocommerce-orders-tracking' ),
									);
									foreach ( $headers as $header_k => $header_v ) {
										?>
                                        <tr>
					...
					case 'import':
                        ...
                        <div>
                            ...
                            <div class="vi-ui small positive message <?php echo esc_attr( self::set( 'view-log-after-import' ) ) ?>"
                                 style="display: none">
                                <div><?php printf( wp_kses_post( __( 'Import completed, please go to <a target="_blank" href="%s">Logs</a> to view import log', 'woocommerce-orders-tracking' ) ), esc_url( admin_url( 'admin.php?page=woocommerce-orders-tracking-logs&log_of=import-tracking' ) ) ) ?></div>
                            </div>
                        </div>
						<?php
						break;
