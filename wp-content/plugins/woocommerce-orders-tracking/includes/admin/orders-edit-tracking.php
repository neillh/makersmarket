...
//					'carrier_name'    => $display_name,
						...
						if ( $track_data['status'] === 'success' ) {
							...
							if ( $track_data['code'] === 4016 ) {
								...
								if ( $track_data['status'] === 'success' ) {
									...
									VI_WOOCOMMERCE_ORDERS_TRACKING_TRACKINGMORE_TABLE::insert( $order_id, $tracking_number, $status, $carrier_slug, $carrier_name, VI_WOOCOMMERCE_ORDERS_TRACKING_DATA::get_shipping_country_by_order_id( $order_id ), $track_info, $description );
								} else {
									VI_WOOCOMMERCE_ORDERS_TRACKING_TRACKINGMORE_TABLE::insert( $order_id, $tracking_number, $status, $carrier_slug, $carrier_name, VI_WOOCOMMERCE_ORDERS_TRACKING_DATA::get_shipping_country_by_order_id( $order_id ), $track_info, $description, '' );
		...
		if ( $tracking_change && 'yes' === $send_sms ) {
			VI_WOOCOMMERCE_ORDERS_TRACKING_ADMIN_SMS::send_sms( $tracking_number, $display_name, $carrier_url, remove_query_arg( array( 'woo_orders_tracking_nonce' ), $tracking_url_import ), $order, $response, $add_new_tracking );
		}
		if ( $quantity_index < 2 ) {
			$item_tracking_data[] = $current_tracking_data;
                            ...
                            <div class="<?php echo esc_attr( self::set( 'edit-tracking-content-body-single' ) ) ?>">
                                ...
                                <div class="<?php echo esc_attr( self::set( array(
                                    ...
                                    <div class="<?php echo esc_attr( self::set( 'edit-tracking-shipping-carrier-wrap' ) ) ?>">
                                        ...
                                        <p class="description wotv-error-tracking-url"><?php esc_html_e( 'The tracking url will not include tracking number if carrier URL does not include ', 'woocommerce-orders-tracking' ) ?>
                                            {tracking_number}</p>
                                    </div>
                                </div>
                            </div>
	...
	public function woocommerce_after_order_itemmeta( $item_id, $item, $product ) {
		...
		if ( $order ) {
			...
			if ( apply_filters( 'vi_woo_orders_tracking_show_tracking_of_order_item', ( $manage_tracking !== 'order_only' || $tracking_number || $carrier_slug ), $item_id, $order_id ) ) {
