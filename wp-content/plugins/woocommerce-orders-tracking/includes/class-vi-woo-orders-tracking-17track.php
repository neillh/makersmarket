...
//					}
							...
							foreach ( $trackings_ as $key_1 => $value_1 ) {
								if ( $value['number'] === $value_1['number'] ) {
									if ( empty( $value_1['carrier'] ) || $value_1['carrier'] == $value['carrier'] ) {
										...
										$trackings[ $key_1 ]['data']                = self::get_track_info( $track['tracking'], true, isset( $track['latest_status']['status'] ) ? $track['latest_status']['status'] : '' );
		 ...
		 * @return array
		 */
		public static function get_track_info( $tracking, $v2 = true, $latest_status = '' ) {
			$track_info = array();
			if ( $v2 ) {
				if ( isset( $tracking['providers'][0]['events'] ) ) {
					if ( is_array( $tracking['providers'][0]['events'] ) && count( $tracking['providers'][0]['events'] ) ) {
						foreach ( $tracking['providers'][0]['events'] as $event ) {
							$track_info[] = array(
								'time'        => $event['time_iso'],
						...
						if ( empty( $track_info[0]['status'] ) ) {
							$track_info[0]['status'] = $latest_status;
						}
					}
				}
			} else {
