...
class StatCollection extends SingletonAbstract {
	...
	public function get_woo_order_stats() {
		...
		$weeks = array(
			'previous_1' => array(
				'end'   => $w_1->modify( '+7 days' )->modify( '-1 second' )->format( 'Y-m-d H:i:s' ),
				'start' => $w_1->format( 'Y-m-d H:i:s' ),
				'week'  => (int) $w_1->format( 'W' ),
				'year'  => (int) $w_1->format( 'o' ),
			),
			'previous_2' => array(
				'end'   => $w_2->modify( '+7 days' )->modify( '-1 second' )->format( 'Y-m-d H:i:s' ),
				'start' => $w_2->format( 'Y-m-d H:i:s' ),
				'week'  => (int) $w_2->format( 'W' ),
				'year'  => (int) $w_2->format( 'o' ),
			),
			'previous_3' => array(
				'end'   => $w_3->modify( '+7 days' )->modify( '-1 second' )->format( 'Y-m-d H:i:s' ),
				'start' => $w_3->format( 'Y-m-d H:i:s' ),
				'week'  => (int) $w_3->format( 'W' ),
				'year'  => (int) $w_3->format( 'o' ),
			),
			'previous_4' => array(
				'end'   => $w_4->modify( '+7 days' )->modify( '-1 second' )->format( 'Y-m-d H:i:s' ),
				'start' => $w_4->format( 'Y-m-d H:i:s' ),
		...
		foreach ( $weeks as $week => $week_details ) {
			...
			$sales_by_date->start_date     = strtotime( $week_details['start'] );
			$sales_by_date->end_date       = strtotime( $week_details['end'] );
			...
			foreach ( $report_data_arr as $key => $value ) {
				if ( in_array( $key, $this->approved_woocomerce_report_stats, true ) ) {
					$weeks[ $week ][ $key ] = $value;
				}
			}
		}
			...
			<div class="updated" style="display: block !important;">
				...
				<p>
					<a href="<?php echo esc_url( $optin_url ); ?>" class="button-secondary">
						<?php echo cfw__( 'Allow', 'checkout-wc' ); ?>
					</a>
					&nbsp;
					<a href="<?php echo esc_url( $optout_url ); ?>" class="button-secondary">
						<?php echo cfw__( 'Do not allow', 'checkout-wc' ); ?>
					</a>
				</p>
			</div>
			<?php
