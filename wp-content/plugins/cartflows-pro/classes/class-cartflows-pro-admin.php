...
class Cartflows_Pro_Admin {
	...
	public function show_admin_notices() {
		...
		add_action( 'admin_notices', array( $this, 'experimental_flow_analytics_notice' ) );
	...
	public function experimental_flow_analytics_notice() {
		...
		if ( ! is_wcf_pro_plan() || ! $this->allowed_screen_for_notices() || ! current_user_can( 'cartflows_manage_flows_steps' ) ) {
			return;
		}

		$show_notice          = get_option( 'wcf_show_flow_analytics_notice', false );
		$exper_flow_analytics = Cartflows_Helper::get_admin_settings_option( '_cartflows_experimental_flow_analytics', false );
		$misc_settings        = admin_url( 'admin.php?page=cartflows&path=settings#miscellaneous_settings' );

		if ( 'yes' === $show_notice && 'enable' !== $exper_flow_analytics ) {
			$message = sprintf(
				/* translators: %1$1s: HTML Tag, %2$2s: HTML Tag, %3$3s: HTML Tag, %4$4s: HTML Tag */
				__( 'Hey! We have improved the way we track your flow analytics for accurate stats and better performance. You can try the new analytic feature right now in beta. Go ahead and give it a try. %1$1s Try now %2$2s', 'cartflows-pro' ),
				'<br><br><a class="button" href="' . $misc_settings . '">',
				'</a>'
			);

			$title = __( 'Introducing a new way to track your flow analytics.', 'cartflows-pro' );

			if ( function_exists( 'wcf_feature_notice' ) ) {
				wcf_feature_notice( $title, $message, 'flow-analytics-notice' );
			}
		}
	}
	...
	public function dismiss_flow_analytics_notice() {
		...
		check_ajax_referer( 'cartflows-dismiss-flow-analytics-notice', 'security' );
		delete_option( 'wcf_show_flow_analytics_notice' );
