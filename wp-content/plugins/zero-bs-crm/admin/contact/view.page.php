                            ...
                            if (count($zbsSocialAccountTypes) > 0 && count($contact_socials) > 0){ 
                              foreach ($zbsSocialAccountTypes as $socialKey => $socialAccType){
                                 if (is_array($contact_socials) && isset($contact_socials[$socialKey]) && !empty($contact_socials[$socialKey])){ 
                                        ...
                                        ?>
                                        <a class="ui mini <?php echo $semanticSocial; ?> button" href="<?php echo esc_url( $socialLink ); ?>" target="_blank" title="<?php echo __('View',"zero-bs-crm").' '.esc_attr( $contact_socials[$socialKey] ); ?>"><i class="large middle aligned ui <?php echo zeroBSCRM_faSocialToSemantic($socialAccType['fa']); ?>" aria-hidden="true" style="padding-top:0"></i></a>
                                    <?php } 
                              }
                            }
                          ?>
							...
							<div class="ui bottom attached tab segment active" data-tab="all-files">
								...
								<thead>
									<tr>
										...
										<th class="center aligned"><?php _e("Uploaded By", 'zero-bs-crm');?></th>
									...
									if (is_array($zbsFiles) && count($zbsFiles) > 0){ 
										...
										<tr>
											<td>
												...
												<?php 
													echo '<a class="" href="' . esc_url( $zbsFile['url'] ) . '" target="_blank" class="ui button basic">' . __( 'View file', 'zero-bs-crm' ) . '</a>';
												?>
											</td>
											...
											<td class="center aligned">
												<?php
													...
													if ( isset( $zbsFile['creation_date'] ) ) {
														echo "<span style='white-space:nowrap;'>";
														echo jpcrm_uts_to_date_str( $zbsFile['creation_date'] );
														echo "</span>";
														$show_empty_marker = false;
													}
													...
													if ( $wp_user !== null && zeroBSCRM_isWPAdmin() ) {
														...
														echo '<br /><br /><a style="font-size: 12px;" href="' . $url . '" target="_blank"><i class="wordpress simple icon"></i> ' . __( 'View WordPress Profile', 'zero-bs-crm' ) . '</a>';
													}
											...
											<td class="center aligned">
												...
												<button class="zbsDelFile ui button basic" data-delurl="<?php echo esc_attr( $zbsFile['url'] ); ?>"><i class="trash alternate icon"></i><?php _e("Delete","zero-bs-crm"); ?></button>
											</td>
										</tr>
										<?php
									...
									<tr id="zbs-no-files-msg" style="display:<?php echo $hasFiles ? 'none' : 'table-row'; ?>">
										<td colspan="4">
											<div class="ui info icon message" id="zbsNoFileResults">
												...
												<p><?php echo sprintf( __( 'This contact does not have any files yet.', "zero-bs-crm" ), $new_file_url ); ?></p>
												</div>
											</div>
										</td>
									</tr>
								...
								<div style="text-align: right;">
									<?php 
										$jpcrm_add_file_url = esc_url( admin_url( 'admin.php?page=' . $zbs->slugs['addnewfile'] ) . '&customer=' . $id  );   
									?>
									<a href="<?php echo $jpcrm_add_file_url ?>" class="ui basic green button" target="_blank">
									<i class="plus square outline icon"></i>
									<?php _e('Add File','zero-bs-crm') ?>
									</a>
								</div>
								...
								if ( !$zbs->hasEntrepreneurBundleMin() ){
									...
									echo sprintf( __( 'Want to allow clients to view and upload files via the Client Portal? <a href="%s" target="_blank">Upgrade to a Bundle</a> (and get Client Portal Pro) to enable this.', 'zero-bs-crm' ), $zbs->urls['upgrade'] . '?utm_content=inplugin-contactview' );
									...
									_e( '<strong>Bundle holder:</strong> Please install the Client Portal Pro extension if you\'d like clients to view and upload their files via the Client Portal.', 'zero-bs-crm' );
