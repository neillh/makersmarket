...
function zeroBSCRM_pages_add_or_edit_file() {
  ...
  global $zbs;
	...
	} else if ( isset( $_POST['fileid'] ) && $_POST['fileid'] !== '' ) {
    ...
    if ($customer > 0) {
		...
		if ( $avatar_mode == "2" || $avatar_mode == "3" ) {
			...
			$view_url_contact = zbsLink( 'view' , -1 , 'zerobs_customer', true ) . $customer;
			$file_owner_html .= '<h4 class="ui image header" style="margin:0px;">';
			$file_owner_html .= '<img src="' . $avatar_url . '" id="profile-picture-img" class="ui mini rounded image" />';
			$file_owner_html .= '<div class="content"><a href="' . $view_url_contact . '">' . $crm_contact['fullname'] . '</a>';
         $file_owner_html .= ! empty( $crm_contact['email'] ) ? '<div class="sub header">' . $crm_contact['email'] . '</div>' : '';
	...
	if (
		...
		if ( $is_adding_file && $customer > 0 ) {
			$supported_types = zeroBS_acceptableFileTypeMIMEArr();

			$tmp_file_info   = finfo_open( FILEINFO_MIME_TYPE );
			$tmp_file_type   = finfo_file( $tmp_file_info, $_FILES['zbsc_file_attachment']['tmp_name'] );
			
			if (
				! in_array( $tmp_file_type, $supported_types ) 
				&& (
					! isset( $supported_types['all'] )
					|| $supported_types['all'] != 1
				)
			) {
				echo "<br>";
				echo "<div class='ui message red' style='margin-right:20px'><i class='icon info'></i> " . __("Error: File type not accepted.", 'zero-bs-crm') . "</div>";
				$error_while_adding = true;
			} else {
				$crm_contact      = $zbs->DAL->contacts->getContact( $customer );
				$contact_dir_info = jpcrm_storage_dir_info_for_contact( $customer );
				if ( !$contact_dir_info ) {
					echo "<div class='ui message red' style='margin-right:20px'><i class='icon info'></i> " . __("Error while retrieving the contact\'s folder.", 'zero-bs-crm') . "</div>";
					$error_while_adding = true;
				} else {
					$upload_path          = $contact_dir_info['files']['path'];
					$upload_folder_exists = jpcrm_create_and_secure_dir_from_external_access( $upload_path, false );
					if ( ! $upload_folder_exists ) {
						echo "<div class='ui message red' style='margin-right:20px'><i class='icon info'></i> " . __("There was an error creating the file upload directory.", 'zero-bs-crm') . "</div>";
						$error_while_adding = true;
					} else {
						$zbs->load_encryption();
						$upload_filename = sprintf( 
							'%s-%s',
							$zbs->encryption->get_rand_hex( 16 ),
							$_FILES['zbsc_file_attachment']['name'] // for admins we are accepting "filename.ext" as provided by $_FILES
						);

						if ( 
							! file_exists( $upload_path . '/' . $upload_filename ) 
							&& move_uploaded_file( $_FILES['zbsc_file_attachment']['tmp_name'], $upload_path . '/' . $upload_filename) 
						) {
							// if it everything went ok adds the file to the list
							$new_upload                  = array();
							$new_upload['file']          = $upload_path . '/' . $upload_filename;
							$new_upload['url']           = $contact_dir_info['files']['url'] . '/' . $upload_filename;
							$new_upload['type']          = $tmp_file_type;
							$new_upload['error']         = '';
							...
							echo "<div class='ui message red' style='margin-right:20px'><i class='icon info'></i> " . __("There was an error uploading the file.", 'zero-bs-crm') . "</div>";
							$error_while_adding = true;
						}
					}
				}
		...
		if ( ! $error_while_adding ) {
			...
			echo "<div class='ui message blue' style='margin-right:20px'><i class='icon info'></i> ".__("Details saved", 'zero-bs-crm') ."</div>";
		}
	}
    ...
    <div class = "ui segment zbs-cp-file-edit-page">
      <?php
		...
		if ( $is_adding_file ) :
      ?>
		<h4><?php _e("Adding New File", 'zero-bs-crm'); ?></h4>
		<?php
		 else : 
		?>
      ...
      <form class="ui form" enctype="multipart/form-data" method="POST" action="#">
          <?php 
            ...
            <label for="zbsc_file_attachment">
					<?php _e("Choose file", 'zero-bs-crm'); ?>
				</label>
				<br />
				<input type="file" id="zbsc_file_attachment" name="zbsc_file_attachment" size="25" class="zbs-dc">
				<em>(<?php _e('Accepted File Types',"zero-bs-crm");?>: <?php echo zeroBS_acceptableFileTypeListStr(); ?>)</em>
				<br />
				</p>
			<?php
			else : 
			?>
				...
				<br/>
				<em><?php echo $file; ?>
				(<a href="<?php echo esc_url( $ourFile['url'] ); ?>" target="_blank"><?php _e("View file", 'zero-bs-crm'); ?></a>)</em>
				</p>
			<?php endif; ?>
          ...
          <div class="block">
             ...
             <div class="right">
                ...
                <div class="price">
                   ...
                   <span class="term"><?php esc_html_e( 'per year', 'zero-bs-crm' ); ?></span>
                </div>
                <div class="zbs-button-wrap">
                   <a href="<?php echo $zbs->urls['upgrade']; ?>" rel="noopener noreferrer" target="_blank" class="upgrade-today btn btn-bta"><?php esc_html_e( 'Upgrade your CRM today', 'zero-bs-crm' ); ?></a>
                </div>
             </div>
             <div class="clear"></div>
          </div> <!-- / .block -->
        ...
        <?php } ;?>
          ...
          <p class="sbhomep">
            ...
            <div class="sbhomep">
              ...
              <p><a class="ui button" href="<?php echo admin_url('admin.php?page='.$zbs->slugs['export-tools'].'&zbswhat=contacts'); ?>">Export Tools</a></p>
            </div>
    </div>
    <div class="ui grid">
    <div class="eight wide column">
