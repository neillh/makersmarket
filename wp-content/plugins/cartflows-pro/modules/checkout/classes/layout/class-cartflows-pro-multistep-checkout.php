...
class Cartflows_Pro_Multistep_Checkout {
	...
	public function update_order_review_template( $fragments ) {
		...
		if ( ! $checkout_id ) {
			...
			$checkout_id = isset( $_GET['wcf_checkout_id'] ) && ! empty( $_GET['wcf_checkout_id'] ) ? intval( wp_unslash( $_GET['wcf_checkout_id'] ) ) : 0;
		}
		...
		if ( wp_doing_ajax() && isset( $_GET['wc-ajax'] ) && 'update_order_review' === $_GET['wc-ajax'] ) {
			ob_start();
	...
	public function add_shipping_section() {
		...
		<?php if ( WC()->cart->needs_shipping() && WC()->cart->show_shipping() ) { ?>
			<div class="wcf-multistep-shipping-methods">
				<h3><?php echo __( 'Shipping method', 'cartflows-pro' ); ?></h3>
			...
			<ul class="wcf-review-details-wrapper">
				<li>
					<div class="wcf-review-details-inner">
						<div role="rowheader" class="wcf-review-detail-label"><?php echo __( 'Contact', 'cartflows-pro' ); ?></div>
						<div class="wcf-review-detail-content contact-details"><?php echo $billing_email; ?></div>
					</div>
					<div role="cell" class="wcf-review-detail-link">
						<a href="javascript:" data-target="billing" class="wcf-step-link"><?php echo __( 'Change', 'cartflows-pro' ); ?></a>
					</div>
				</li>
			</ul>
		</div>
		...
		if ( ! WC()->cart->needs_shipping() && 'no' === $show_order_field ) {
			echo '<div class="wcf-multistep-checkout-breadcrumb">
					<span class="wcf-checkout-breadcrumb-list">
						<li class="wcf-checkout-breadcrumb information-step wcf-current-step"><a href="" data-tab="billing" class="wcf-current-step">' . __( 'Information', 'cartflows-pro' ) . '</a></li>
						<li class="wcf-checkout-breadcrumb payment-step"><a href="" data-tab="payment">' . __( 'Payment', 'cartflows-pro' ) . '</a></li>
					</span>
				</div>';
			...
			echo '<div class="wcf-multistep-checkout-breadcrumb">
						<span class="wcf-checkout-breadcrumb-list">
							<li class="wcf-checkout-breadcrumb information-step wcf-current-step"><a href="" data-tab="billing" class="wcf-current-step">' . __( 'Information', 'cartflows-pro' ) . '</a></li>
							<li class="wcf-checkout-breadcrumb shipping-step"><a href="" data-tab="shipping">' . $breadcrumb_text . '</a></li>
							<li class="wcf-checkout-breadcrumb payment-step"><a href="" data-tab="payment">' . __( 'Payment', 'cartflows-pro' ) . '</a></li>
						</span>
				</div>';
		}
	...
	public function add_back_nav_button_for_payment( $html ) {
		...
		if ( ! $this->is_multistep_checkout_layout( $checkout_id ) || ( wp_doing_ajax() && isset( $_REQUEST['action'] ) && 'update_order_review' !== $_REQUEST['action'] ) ) {
			return $html;
		}
	...
	public function add_navigation_buttons( $checkout_id ) {
		...
		echo "<div class='wcf-multistep-nav-btn-group'>
				<a href='' class='wcf-multistep-nav-back-btn' data-target=''>" . __( '« Back to information', 'cartflows-pro' ) . "</a>
				<span class='wcf-multistep-nav-next-btn' data-target='" . $data_target . "'>" . $second_step_btn_text . '</span>
			</div></div>';
				...
				<div class="woocommerce-billing-fields-custom">
					<h3><?php echo wp_kses_post( apply_filters( 'cartflows_woo_customer_info_text', esc_html__( 'Customer information', 'cartflows-pro' ) ) ); ?>
						<?php if ( ! is_user_logged_in() && $is_allow_login ) { ?>
							<div class="woocommerce-billing-fields__customer-login-label"><?php /* translators: %1$s: Link HTML start, %2$s Link HTML End */ echo sprintf( __( 'Already have an account? %1$1s Log in%2$2s', 'cartflows-pro' ), '<a href="javascript:" class="wcf-customer-login-url">', '</a>' ); ?></div>
						<?php } ?>
					</h3>
					...
					if ( ! is_user_logged_in() ) {
						...
						if ( 'yes' === get_option( 'woocommerce_enable_checkout_login_reminder' ) ) {
							...
							<?php
									...
									echo "<a href='$lost_password_url' class='wcf-customer-login-lost-password-url'>" . esc_html( __( 'Lost your password?', 'cartflows-pro' ) ) . '</a>';
							?>
								</div>
							<?php
						...
						<?php } ?>
					<?php } else { ?>
								<div class="wcf-logged-in-customer-info"> <?php /* translators: %1$s: username, %2$s emailid */ echo apply_filters( 'cartflows_logged_in_customer_info_text', sprintf( __( ' Welcome Back %1$s ( %2$s )', 'cartflows-pro' ), esc_attr( $current_user_name ), esc_attr( $current_user_email ) ) ); ?>
	...
	public function custom_address_section() {
		...
		<div class="wcf-payment-step-review-details">
			<ul class="wcf-review-details-wrapper">
				...
				<li>
					<div class="wcf-review-details-inner">
						<div role="rowheader" class="wcf-review-detail-label"><?php echo __( 'Address', 'cartflows-pro' ); ?></div>
						<div class="wcf-review-detail-content shipping-details"><?php echo $ship_to; ?></div>
					</div>
					<div role="cell" class="wcf-review-detail-link">
						<a href="javascript:" data-target=<?php echo $shipping_data_target; ?> class="wcf-step-link"><?php echo __( 'Change', 'cartflows-pro' ); ?></a>
					</div>
				</li>
			</ul>
		</div>
