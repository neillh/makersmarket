			...
			<script type="text/template" id="tmpl-cartflows-deactivate-license">
				<table class="widefat">
					<tr class="cartflows-row">
						<td class="cartflows-heading"><?php esc_html_e( 'License Key', 'cartflows-pro' ); ?></td>
						<td class="cartflows-content">
							<input type="password" placeholder="<?php esc_attr( '******************' ); ?>" class="regular-text license_key" value="" autocomplete="off" text="" readonly="readonly">
...
if ( ! function_exists( 'cartflows_pro_license_request' ) ) :
	...
	function cartflows_pro_license_request( $license_key = '', $site_id = 1, $request_type = '' ) {
		...
		// Default response array. Will be send when no request type is provided in the request.
		$request = array(
			'success'    => false,
			'error_code' => __( 'No request type', 'cartflows-pro' ),
			'error'      => __( 'Request type not provided', 'cartflows-pro' ),
			'data'       =>
				array(
					'error_code' => __( 'No request type', 'cartflows-pro' ),
					'error'      => __( 'Request type not provided', 'cartflows-pro' ),
