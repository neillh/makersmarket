...
class BWFAN_API_Carts_View_Tasks extends BWFAN_API_Base {
	...
	public function process_api_call() {
		...
		if ( 'recovered' === $type ) {
			...
			$cart_details['email'] = get_post_meta( $order_id, '_billing_email', true );

			$basic_details['email']  = $cart_details['email'];
			$basic_details['f_name'] = get_post_meta( $order_id, '_billing_first_name', true );
			$basic_details['l_name'] = get_post_meta( $order_id, '_billing_last_name', true );
		} else {
