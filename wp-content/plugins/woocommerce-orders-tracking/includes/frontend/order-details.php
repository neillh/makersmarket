...
class VI_WOOCOMMERCE_ORDERS_TRACKING_FRONTEND_ORDER_DETAILS {
	...
	public function woocommerce_order_details_before_order_table( $order ) {
		if ( is_order_received_page() && self::$settings->get_params( 'default_track_info_number' ) && 'before_order_table' === self::$settings->get_params( 'default_track_info_position' ) ) {
	...
	public function woocommerce_order_details_after_order_table( $order ) {
		if ( is_order_received_page() && self::$settings->get_params( 'default_track_info_number' ) && 'after_order_table' === self::$settings->get_params( 'default_track_info_position' ) ) {
			$this->add_default_tracking_number_for_new_order( $order );
		}
	}
						...
						if ( count( $track_args['tracking'] ) ) {
							...
							if ( $tracking_url_show ) {
								...
								if ( $track_args['status'] ) {
									...
									$title             = sprintf( esc_attr__( 'Shipment status: %s', 'woocommerce-orders-tracking' ), self::$settings->get_status_text_by_service_carrier( $track_args['status'] ) );
								}
								?>
	...
	protected function print_tracking_number( $current_tracking_data, $item_id, $order_id, $order, &$tracking_list ) {
		...
		if ( $tracking_number && ! in_array( $tracking_number, $tracking_list ) ) {
			...
			if ( $tracking_status ) {
				...
				$title             = sprintf( esc_attr__( 'Shipment status: %s', 'woocommerce-orders-tracking' ), self::$settings->get_status_text_by_service_carrier( $current_tracking_data['status'] ) );
			}
			?>
            <div class="<?php echo esc_attr( self::set( $container_class ) ) ?>"
                 title="<?php echo $title ?>">
