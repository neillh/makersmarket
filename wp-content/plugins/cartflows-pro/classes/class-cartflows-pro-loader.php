...
if ( ! class_exists( 'Cartflows_Pro_Loader' ) ) {
	...
	final class Cartflows_Pro_Loader {
		...
		public function define_constants() {
			...
			define( 'CARTFLOWS_PRO_VER', '1.11.9' );
			...
			define( 'CARTFLOWS_PRO_REQ_CF_VER', '1.11.8' );
		...
		public function load_core_components() {
			...
			// Tracking feature is available in pro version. But we are tracking it in all versions.
			$exper_flow_analytics = Cartflows_Helper::get_admin_settings_option( '_cartflows_experimental_flow_analytics', false );

			if ( 'enable' === $exper_flow_analytics ) {
				include_once CARTFLOWS_PRO_DIR . 'modules/tracking/class-cartflows-pro-analytics-tracking-beta.php';
			} else {
		...
		public function licence_setup() {
			...
			if ( class_exists( 'WC_AM_Client_25' ) ) {
				...
				global $wcam_lib;
				$wcam_lib = new WC_AM_Client_25( CARTFLOWS_PRO_FILE, CARTFLOWS_PRO_PRODUCT_ID, CARTFLOWS_PRO_VER, 'plugin', CARTFLOWS_SERVER_URL, 'cartflows' );
			}
