...
if ( ! class_exists( 'VI_WOOCOMMERCE_ORDERS_TRACKING_ADMIN_WEBHOOKS' ) ) {
	class VI_WOOCOMMERCE_ORDERS_TRACKING_ADMIN_WEBHOOKS {
		...
		public function admin_menu() {
			add_submenu_page( 'woocommerce-orders-tracking', esc_html__( 'Webhooks', 'woocommerce-orders-tracking' ), esc_html__( 'Webhooks', 'woocommerce-orders-tracking' ), VI_WOOCOMMERCE_ORDERS_TRACKING_DATA::get_required_capability( 'webhooks' ), 'woocommerce-orders-tracking-webhooks', array(
				$this,
				'page_callback'
			) );
		}
                        ...
                        <ul class="list">
							...
							switch ( $service_carrier_type ) {
								case 'trackingmore':
                                    ...
                                    <li><?php esc_html_e( 'Copy Webhook url below and paste it to Webhook URL field of your Webhook Notification Settings', 'woocommerce-orders-tracking' ); ?></li>
								...
								case 'aftership':
                                    ...
                                    <li><?php esc_html_e( 'Copy the AfterShip Webhook secret and paste it to respective Webhook secret field below', 'woocommerce-orders-tracking' ); ?></li>
								...
								case '17track':
									?>
                                    <li><?php _e( 'Copy below webhook URL and paste to WebHook field in <a href="https://api.17track.net/en/admin/settings">17track settings</a>', 'woocommerce-orders-tracking' ); ?></li>
                                    <li><?php esc_html_e( 'Under Notification settings, select statuses to receive notification when tracking information changes', 'woocommerce-orders-tracking' ); ?></li>
									<?php
					...
					if ( self::support_webhooks( $service_carrier_type ) ) {
                        ...
                        <form class="vi-ui form" method="post">
                            ...
                            <table class="form-table">
                                ...
                                <tr>
                                    <th>
                                        <label for="<?php echo esc_attr( self::set( 'webhooks_debug' ) ) ?>"><?php esc_html_e( 'Enable Debug', 'woocommerce-orders-tracking' ) ?></label>
                                    </th>
                                    <td>
                                        <div class="vi-ui toggle checkbox">
                                            <input type="checkbox"
                                        ...
                                        <div class="description"><?php esc_html_e( 'If enabled, webhook data will be logged for debugging purpose', 'woocommerce-orders-tracking' ) ?></div>
                                    </td>
                                </tr>
								<?php
								if ( $option_field ) {
				...
				if ( $timeStr && $signature ) {
					if ( self::verify_trackingmore( $timeStr, $user_email, $signature ) ) {
						if ( isset( $input['data'] ) && is_array( $input['data'] ) && count( $input['data'] ) ) {
							...
							$carrier_id      = isset( $tracking['courier_code'] ) ? $tracking['courier_code'] : ( isset( $tracking['carrier_code'] ) ? $tracking['carrier_code'] : '' );
							if ( $tracking_number && $carrier_id ) {
								...
								$last_event     = isset( $tracking['latest_event'] ) ? $tracking['latest_event'] : ( isset( $tracking['lastEvent'] ) ? $tracking['lastEvent'] : '' );
								$status         = isset( $tracking['delivery_status'] ) ? $tracking['delivery_status'] : ( isset( $tracking['status'] ) ? $tracking['status'] : '' );
								...
								if ( $carrier_slug ) {
									...
									if ( count( $results ) ) {
										...
										if ( ! $log ) {
											$log = sprintf( esc_html__( 'New status received for tracking number %s: %s', 'woocommerce-orders-tracking' ), $tracking_number, $convert_status );
										}
										...
										if ( ! VI_WOOCOMMERCE_ORDERS_TRACKING_TRACKINGMORE_TABLE::update_by_tracking_number( $tracking_number, $status, $carrier_slug, false, false, $track_info, $last_event ) ) {
											foreach ( $order_ids as $order_id ) {
												VI_WOOCOMMERCE_ORDERS_TRACKING_TRACKINGMORE_TABLE::insert( $order_id, $tracking_number, $status, $carrier_slug, '', VI_WOOCOMMERCE_ORDERS_TRACKING_DATA::get_shipping_country_by_order_id( $order_id ), $track_info, $last_event, '' );
											}
										}
									}
								} else {
		...
		private static function handle_data_17track( $carrier_no, $tracking_number, $track, $tracking_service, $v2 = true ) {
			...
			if ( count( $carrier_slugs ) ) {
				...
				if ( count( $results ) ) {
					if ( $v2 ) {
						$track_info = VI_WOOCOMMERCE_ORDERS_TRACKING_17TRACK::get_track_info( $track['track_info']['tracking'], true, isset( $track['track_info']['latest_status']['status'] ) ? $track['track_info']['latest_status']['status'] : '' );
						$last_event = '';
						if ( $track_info ) {
						...
						$original_country = isset( $track['track_info']['shipping_info']['shipper_address']['country'] ) ? $track['track_info']['shipping_info']['shipper_address']['country'] : '';
						$dest_country     = isset( $track['track_info']['shipping_info']['recipient_address']['country'] ) ? $track['track_info']['shipping_info']['recipient_address']['country'] : '';
					} else {
					...
					foreach ( $order_ids_by_carriers as $carrier_slug => $order_ids_by_carrier ) {
						if ( ! VI_WOOCOMMERCE_ORDERS_TRACKING_TRACK_INFO_TABLE::update_by_tracking_number( $tracking_number, $carrier_slug, $tracking_service, $status, $track_info, $last_event, '', $original_country, $dest_country ) ) {
							foreach ( $order_ids_by_carrier as $order_id ) {
								VI_WOOCOMMERCE_ORDERS_TRACKING_TRACK_INFO_TABLE::insert( $tracking_number, $order_id, $carrier_slug, $tracking_service, $status, $track_info, $last_event, '', '', $original_country, $dest_country );
							}
						}
					}
				} else {
						...
						if ( $tracking_number ) {
							...
							if ( count( $carrier_slugs ) ) {
								...
								if ( count( $results ) ) {
									self::handle_found_items( $tracking_service, $tracking_number, $status, $carrier_slugs, $results, $order_ids_by_carriers );
									foreach ( $order_ids_by_carriers as $carrier_slug => $order_ids_by_carrier ) {
										if ( ! VI_WOOCOMMERCE_ORDERS_TRACKING_TRACK_INFO_TABLE::update_by_tracking_number( $tracking_number, $carrier_slug, $tracking_service, $status, $track_info, $last_event, $tracking['expected_delivery'], $tracking['origin_country_iso3'], $tracking['destination_country_iso3'] ) ) {
											foreach ( $order_ids_by_carrier as $order_id ) {
												VI_WOOCOMMERCE_ORDERS_TRACKING_TRACK_INFO_TABLE::insert( $tracking_number, $order_id, $carrier_slug, $tracking_service, $status, $track_info, $last_event, $tracking['expected_delivery'], false, $tracking['origin_country_iso3'], $tracking['destination_country_iso3'] );
											}
										}
									}
								}
						...
						if ( $tracking_number ) {
							...
							if ( count( $carrier_slugs ) ) {
								...
								if ( count( $results ) ) {
									self::handle_found_items( $tracking_service, $tracking_number, $status, $carrier_slugs, $results, $order_ids_by_carriers );
									foreach ( $order_ids_by_carriers as $carrier_slug => $order_ids_by_carrier ) {
										if ( ! VI_WOOCOMMERCE_ORDERS_TRACKING_TRACK_INFO_TABLE::update_by_tracking_number( $tracking_number, $carrier_slug, $tracking_service, $status, $track_info, $last_event, $tracking['est_delivery_date'] ) ) {
											foreach ( $order_ids_by_carrier as $order_id ) {
												VI_WOOCOMMERCE_ORDERS_TRACKING_TRACK_INFO_TABLE::insert( $tracking_number, $order_id, $carrier_slug, $tracking_service, $status, $track_info, $last_event, $tracking['est_delivery_date'] );
											}
										}
									}
								}
		...
		private static function handle_found_items( $tracking_service, $tracking_number, $status, $carrier_slugs, $results, &$order_ids_by_carriers ) {
			...
			if ( ! $log ) {
				...
				$log = sprintf( esc_html__( 'New status received for tracking number %s: %s', 'woocommerce-orders-tracking' ), $tracking_number, ( isset( $shipment_statuses[ $status ] ) ? $shipment_statuses[ $status ] : $status ) );
			}
			self::send_email_based_on_status( $send_email, $status, $order_ids, $log );
			self::wc_log( $log );
		}
		...
		private static function update_order_status( $tracking_number, $convert_status, $order_ids, $order_item_ids, $changed_order_ids, &$log ) {
			...
			$changed_orders      = VI_WOOCOMMERCE_ORDERS_TRACKING_ADMIN_ORDERS_TRACK_INFO::update_order_status( $convert_status, $order_ids, $order_item_ids, $change_order_status, 'delivered', $changed_order_ids );
