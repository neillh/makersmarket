...
class BWFAN_Traversal_Controller {
	...
	/**
	 * Get previous step id of current step
	 *
	 * @param $target_step
	 *
	 * @return int|mixed|string
	 */
	public function get_previous_step_id( $target_step ) {
		if ( empty( $target_step ) ) {
			return 0;
		}

		$target_step_node_id = '';
		foreach ( $this->steps as $step ) {
			if ( isset( $step['stepId'] ) && ( absint( $step['stepId'] ) === absint( $target_step ) ) ) {
				$target_step_node_id = $step['id'];
				break;
			}
		}
		if ( empty( $target_step_node_id ) ) {
			return 0;
		}

		$prev_step_node_id = array_search( $target_step_node_id, $this->links );

		/** If previous step node is start */
		if ( 'start' === $prev_step_node_id ) {
			return 'start';
		}
		if ( ! isset( $this->steps[ $prev_step_node_id ] ) || ! isset( $this->steps[ $prev_step_node_id ]['stepId'] ) ) {
			return 0;
		}

		return $this->steps[ $prev_step_node_id ]['stepId'];
	}
	 ...
	 *
	 * @param $ins BWFAN_Automation_Controller
	 *
	 * @return void
	 */
	public function maybe_traverse_back( $ins ) {
		global $wpdb;
