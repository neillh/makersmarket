...
class VI_WOOCOMMERCE_ORDERS_TRACKING_ADMIN_EMAIL {
		...
		if ( ! isset( self::$default_tracking[ $order_id ] ) ) {
			...
			if ( $_wot_tracking_number && $_wot_tracking_carrier ) {
				...
				if ( is_array( $carrier ) && count( $carrier ) && empty( $carrier['digital_delivery'] ) ) {
					...
					$tracking_url_show                   = apply_filters( 'vi_woo_orders_tracking_current_tracking_url_show', self::$settings->get_url_tracking( $carrier_url, $_wot_tracking_number, $_wot_tracking_carrier, $order->get_shipping_postcode(), false, false, $order_id ), '', $order_id );
        ...
        <table class="<?php echo esc_attr( VI_WOOCOMMERCE_ORDERS_TRACKING_DATA::set( 'table-container' ) ) ?>">
			...
			if ( count( $updated_items ) ) {
				...
				foreach ( $updated_items as $item ) {
					...
					self::print_item_tracking( $item['order_item_name'], $item['tracking_number'], $item['carrier_name'], $carrier_url, $item['tracking_url'], $email_column_tracking_number, $email_column_carrier_name, $email_column_tracking_url );
				}
				if ( $email_send_all_order_items ) {
					foreach ( $line_items as $item_id => $line_item ) {
						if ( ! in_array( $item_id, $order_item_ids ) ) {
							...
							self::print_tracking_row( $current_tracking_data, $item_id, $line_item, $order_id, $order, $email_column_tracking_number, $email_column_carrier_name, $email_column_tracking_url, $g_tracking, true );
						}
	 ...
	 */
	protected static function print_item_tracking( $item_name, $tracking_number, $carrier_name, $carrier_url, $tracking_url, $email_column_tracking_number, $email_column_carrier_name, $email_column_tracking_url ) {
		?>
        <tr>
	 ...
	 */
	protected static function print_tracking_row( $current_tracking_data, $item_id, $line_item, $order_id, $order, $email_column_tracking_number, $email_column_carrier_name, $email_column_tracking_url, &$g_tracking, $allow_empty = true ) {
		global $woo_orders_tracking_items;
		...
		if ( $tracking_number || $digital_delivery ) {
			...
			if ( isset( $tracking_url ) ) {
				...
				if ( $tracking_number ) {
					$tracking_url_show = apply_filters( 'vi_woo_orders_tracking_current_tracking_url_show', self::$settings->get_url_tracking( $carrier_url, $tracking_number, $carrier_slug, $order->get_shipping_postcode(), false, false, $order_id ), $item_id, $order_id );
				} else {
			...
			self::print_item_tracking( $line_item->get_name(), $tracking_number, $display_name, $carrier_url_show, $tracking_url_show, $email_column_tracking_number, $email_column_carrier_name, $email_column_tracking_url );
		} elseif ( $allow_empty ) {
			$woo_orders_tracking_items[] = array(
				'order_item_name' => $line_item->get_name(),
				'tracking_number' => '',
