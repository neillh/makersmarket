...
if (isset($_POST['editwplf']) && zeroBSCRM_isZBSAdminOrAdmin()){
    ...
    $updatedSettings['menulayout'] = 2; if(isset($_POST['wpzbscrm_menulayout']) && !empty($_POST['wpzbscrm_menulayout'])) $updatedSettings['menulayout'] = (int)sanitize_text_field($_POST['wpzbscrm_menulayout']);
...
if (!$confirmAct){
    ...
    <form method="post" action="?page=<?php echo $zbs->slugs['settings']; ?>">
        ...
        <table class="table table-bordered table-striped wtab">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td style="width:540px">
                        <input type="text" class="wpinput form-control" name="wpzbscrm_secondaddresslabel" id="pzbscrm_secondaddresslabel" value="<?php echo empty( $settings['secondaddresslabel'] ) ? '' : esc_attr( $settings['secondaddresslabel'] ); ?>" placeholder="<?php _e("Second Address (if left blank)","zero-bs-crm"); ?>" />
                    </td>
                </tr>
        ...
        <table class="table table-bordered table-striped wtab">
            ...
            <tr>
                <td class="wfieldname">
                    <label for="jpcrm_showpoweredby_public"><?php _e( 'Show public credits', 'zero-bs-crm' ); ?>:</label><br />
                    <?php _e( 'Love Jetpack CRM and willing to share? Show us some love by displaying "Powered by Jetpack CRM" footers on the CRM login page, CRM forms, CRM emails, and the Client Portal.', 'zero-bs-crm' ); ?>
                </td>
                <td style="width:540px">
            ...
            <tr>
                <td class="wfieldname">
                    <label for="jpcrm_showpoweredby_admin"><?php _e( 'Show admin credits', 'zero-bs-crm' ); ?>:</label><br />
                    <?php _e( 'Show "Powered by Jetpack CRM" footers on backend admin pages.', 'zero-bs-crm' ); ?>
                </td>
                <td style="width:540px">
        ...
        <table class="table table-bordered table-striped wtab">
            ...
            <tr>
                <td class="wfieldname"><label><?php _e('Allow CRM team members to upload all filetypes',"zero-bs-crm"); ?>:</label><br /><?php _e('This setting overrides the above accepted upload file types, and will allow CRM team members to upload any files to the CRM. For security reasons, we don\'t recommend doing this unless necessary.',"zero-bs-crm"); ?></td>
                <td style="width:540px">
                    <input type="checkbox" class="winput form-control" name="<?php echo 'wpzbscrm_ft_all'; ?>" id="<?php echo 'wpzbscrm_ft_all'; ?>" value="1"<?php if (isset($settings['filetypesupload']) && isset($settings['filetypesupload']['all']) && $settings['filetypesupload']['all'] == "1") echo ' checked="checked"'; ?> /> Accept all files from admins<br />
                </td>
            </tr>
