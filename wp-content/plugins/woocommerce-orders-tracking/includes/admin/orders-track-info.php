...
class VI_WOOCOMMERCE_ORDERS_TRACKING_ADMIN_ORDERS_TRACK_INFO {
	...
	public static function enqueue_main_script() {
		...
		$css = '.woo-orders-tracking-tracking-number-container-delivered .woo-orders-tracking-tracking-number .woo-orders-tracking-tracking-number-value{color:' . self::$settings->get_params( 'timeline_track_info_status_background_delivered' ) . '}';
		$css .= '.woo-orders-tracking-tracking-number-container-pickup .woo-orders-tracking-tracking-number .woo-orders-tracking-tracking-number-value{color:' . self::$settings->get_params( 'timeline_track_info_status_background_pickup' ) . '}';
		$css .= '.woo-orders-tracking-tracking-number-container-transit .woo-orders-tracking-tracking-number .woo-orders-tracking-tracking-number-value{color:' . self::$settings->get_params( 'timeline_track_info_status_background_transit' ) . '}';
		$css .= '.woo-orders-tracking-tracking-number-container-pending .woo-orders-tracking-tracking-number .woo-orders-tracking-tracking-number-value{color:' . self::$settings->get_params( 'timeline_track_info_status_background_pending' ) . '}';
		$css .= '.woo-orders-tracking-tracking-number-container-alert .woo-orders-tracking-tracking-number .woo-orders-tracking-tracking-number-value{color:' . self::$settings->get_params( 'timeline_track_info_status_background_alert' ) . '}';
		...
		wp_localize_script(
			...
			array(
				...
				'i18n_order_row'                      => esc_html__( 'Tracking of the whole order: use this if this order only uses 1 tracking number for all items', 'woocommerce-orders-tracking' ),
				...
				'i18n_error_api'                      => sprintf( esc_html__( 'Error adding tracking number {tracking_number} to %s', 'woocommerce-orders-tracking' ), VI_WOOCOMMERCE_ORDERS_TRACKING_DATA::service_carriers_list( self::$settings->get_params( 'service_carrier_type' ) ) ),
	...
	public static function edit_tracking_content_footer() {
        ...
        <div><?php esc_html_e( 'To add a new carrier or turn on/off a carrier, please go to ', 'woocommerce-orders-tracking' ) ?>
            <a target="_blank"
               href="<?php echo esc_url( admin_url( 'admin.php?page=woocommerce-orders-tracking#shipping_carriers' ) ) ?>"><?php esc_html_e( 'Carriers settings', 'woocommerce-orders-tracking' ) ?></a></label>
        </div>
        <div>
            <span class="button button-primary <?php echo esc_attr( self::set( 'edit-tracking-button-save-all' ) ) ?>"><?php esc_html_e( 'Save', 'woocommerce-orders-tracking' ) ?></span>
		...
		if ( $order ) {
			...
			if ( count( $line_items ) ) {
                ...
                <div class="<?php echo esc_attr( self::set( 'tracking-number-column-container' ) ) ?>">
					...
					$tracking_carrier = get_post_meta( $order_id, '_wot_tracking_carrier', true );
					if ( $manage_tracking !== 'items_only' || ( $tracking_number && $tracking_carrier ) ) {
						self::print_tracking_row( array(
							...
							'carrier_name'    => '',
							'carrier_type'    => '',
							'status'          => get_post_meta( $order_id, '_wot_tracking_status', true ),
							'time'            => time(),
					...
					foreach ( $line_items as $item_id => $line_item ) {
						...
						if ( apply_filters( 'vi_woo_orders_tracking_show_tracking_of_order_item', ( $manage_tracking !== 'order_only' || $current_tracking_data['tracking_number'] || $current_tracking_data['carrier_slug'] ), $item_id, $order_id ) ) {
								...
								for ( $i = 0; $i < $quantity - 1; $i ++ ) {
									if ( isset( $item_tracking_data[ $i ] ) ) {
										...
										if ( $track_per_quantity || $current_tracking_data['tracking_number'] || $current_tracking_data['carrier_slug'] ) {
											self::print_tracking_row( $current_tracking_data, $line_item, $order_id, $order, $transID, $paypal_method, $paypal_added_trackings, $tracking_list, $i + 2 );
										}
									}
								}
							} else if ( $track_per_quantity ) {
								for ( $i = 0; $i < $quantity - 1; $i ++ ) {
									$current_tracking_data = array(
										'tracking_number' => '',
										'carrier_slug'    => '',
	 ...
	 */
	protected static function print_tracking_row( $current_tracking_data, $line_item, $order_id, $order, $transID, $paypal_method, $paypal_added_trackings, &$tracking_list, $quantity_index = 1 ) {
		...
		if ( $tracking_number && ! in_array( $tracking_number, $tracking_list ) ) {
            ...
            <span class="<?php echo esc_attr( self::set( 'tracking-number' ) ) ?>"
                  title="<?php printf( esc_attr__( 'Tracking carrier %s', 'woocommerce-orders-tracking' ), esc_attr( $carrier_name ) ) ?>"><span
                        ...
                        class="<?php echo esc_attr( self::set( 'tracking-carrier' ) ) ?>"><?php printf( esc_html__( 'by %s', 'woocommerce-orders-tracking' ), '<strong>' . $carrier_name . '</strong>' ) ?></span></span>
			<?php
		...
		if ( count( $results ) ) {
			...
			foreach ( $results as $result ) {
				if ( $result['order_item_id'] ) {
					...
					update_post_meta( $result['order_id'], '_wot_tracking_status', $status );
				}
			}
			...
			foreach ( $order_ids as $order_id ) {
				...
				if ( $order ) {
					...
					if ( count( $line_items ) ) {
						...
						if ( apply_filters( "vi_woo_orders_tracking_is_order_{$shipment_status}", ( $shipment_status_count === $tracking_fields_count ), $order ) ) {
	...
	public function refresh_tracking_number_column() {
		$action_nonce = isset( $_GET['action_nonce'] ) ? wp_unslash( sanitize_text_field( $_GET['action_nonce'] ) ) : '';
		if ( ! wp_verify_nonce( $action_nonce, 'vi_wot_item_action_nonce' ) ) {
			return;
		}
		...
		$update_settings     = isset( $_GET['update_settings'] ) ? sanitize_text_field( $_GET['update_settings'] ) : '';
		$change_order_status = isset( $_GET['change_order_status'] ) ? sanitize_text_field( $_GET['change_order_status'] ) : '';
		$send_email          = isset( $_GET['send_email'] ) ? sanitize_text_field( $_GET['send_email'] ) : '';
		$send_sms            = isset( $_GET['send_sms'] ) ? sanitize_text_field( $_GET['send_sms'] ) : '';
		$response            = array(
			'status'  => 'error',
			'message' => '',
			'html'    => '',
		...
		if ( $order_id && wp_verify_nonce( $action_nonce, 'vi_wot_item_action_nonce' ) ) {
			if ( $update_settings ) {
				if ( $change_order_status != self::$settings->get_params( 'order_status' ) || $send_email != self::$settings->get_params( 'email_enable' ) || $send_email != self::$settings->get_params( 'sms_enable' ) ) {
					...
					case 'trackingmore':
						...
						if ( ! count( $tracking_from_db ) ) {
							...
							if ( $track_data['status'] === 'success' ) {
								...
								VI_WOOCOMMERCE_ORDERS_TRACKING_TRACKINGMORE_TABLE::insert( $order_id, $tracking_number, $status, $carrier_slug, $carrier_name, VI_WOOCOMMERCE_ORDERS_TRACKING_DATA::get_shipping_country_by_order_id( $order_id ), $track_info, '' );
							} else {
								if ( $track_data['code'] === 4016 ) {
									...
									VI_WOOCOMMERCE_ORDERS_TRACKING_TRACKINGMORE_TABLE::insert( $order_id, $tracking_number, $status, $carrier_slug, $carrier_name, VI_WOOCOMMERCE_ORDERS_TRACKING_DATA::get_shipping_country_by_order_id( $order_id ), $track_info, $description, $modified_at );
								} else {
									$response['status']  = 'error';
									$response['message'] = $track_data['data'];
								}
							...
							if ( $convert_status !== 'delivered' ) {
								...
								if ( $track_data['status'] === 'success' ) {
									if ( count( $track_data['data'] ) ) {
										...
										$last_event                 = array_shift( $track_data['data'] );
										$status                     = $last_event['status'];
										VI_WOOCOMMERCE_ORDERS_TRACKING_TRACKINGMORE_TABLE::update( $tracking_from_db['id'], $order_id, $status, $carrier_slug, $carrier_name, VI_WOOCOMMERCE_ORDERS_TRACKING_DATA::get_shipping_country_by_order_id( $order_id ), $track_info, $description );
	...
	public function posts_where( $where, $wp_query ) {
		...
		if ( isset( $_GET['filter_action'] ) && $tracking_code && $post_type === 'shop_order' ) {
			$where .= $wpdb->prepare( " AND ((wotg_woocommerce_order_itemmeta.meta_key='_vi_wot_order_item_tracking_data' AND wotg_woocommerce_order_itemmeta.meta_value like %s) or ($wpdb->postmeta.meta_key='_wot_tracking_number' AND $wpdb->postmeta.meta_value=%s))", '%' . $wpdb->esc_like( $tracking_code ) . '%', $tracking_code );
