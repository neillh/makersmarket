...
class Cartflows_Pro_Offer_Order_Meta {
	...
	public function __construct() {
		...
		if ( is_admin() ) {
			...
			add_action( 'woocommerce_admin_order_data_after_order_details', array( $this, 'offer_linked_orders' ), 10, 1 );
		}
	}
	...
	public function offer_linked_orders( $order ) {
		...
		if ( 'yes' === $is_cartflows_offer ) {
			...
			if ( ! empty( $parent_order_id ) ) {
				...
				$parent_order_html .= '<span style= "display: block;"><a href="' . $parent_order->get_edit_order_url() . '"><strong>#' . esc_attr( $order_number ) . '</strong></a></span>';
			...
			if ( ! empty( $child_orders ) ) {
				...
				foreach ( $child_orders as $child_id => $data ) {
					...
					$child_order_html .= '<span style= "display: block;"><a href="' . $child_order->get_edit_order_url() . '"><strong>#' . esc_attr( $order_number ) . '</strong></a>&nbsp;&hyphen;&nbsp;' . $tag . '</span>';
				}
