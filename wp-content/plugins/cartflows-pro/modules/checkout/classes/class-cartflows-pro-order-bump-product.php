...
class Cartflows_Pro_Order_Bump_Product {
	...
	public function __construct() {
		...
		add_action( 'cartflows_checkout_after_configure_cart', array( $this, 'add_prechecked_order_bumps' ), 10, 1 );

		$this->setup_events_conditional_order_bumps();
	}
	...
	public function setup_events_conditional_order_bumps() {
		...
		foreach ( $product_option_events as $event ) {
			add_action( $event, array( $this, 'order_bump_fragments_actions' ), 20 );
		}
	}
	...
	public function get_single_order_bump_markup( $checkout_id, $order_bump_data ) {
		...
		if ( is_array( $order_bump_data ) && ! empty( $order_bump_data ) ) {
			...
			$order_bump_checked = isset( $order_bump_data['default_state'] ) && 'yes' === $order_bump_data['default_state'] ? true : $order_bump_checked;
	...
	public function prepare_ob_data( $checkout_id, $ob_id ) {
		...
		$ob_qty = ! empty( $_POST['_bump_offer_qty'] ) ? intval( $_POST['_bump_offer_qty'] ) : intval( $required_ob_data['quantity'] );
	...
	public function add_prechecked_order_bumps( $checkout_id ) {
		...
		if ( ! empty( $order_bumps ) && ! ( wp_doing_ajax() || isset( $_GET['wc-ajax'] ) ) ) {
