...
Your sincerely',
	...
	public static function convert_status( $status ) {
		if ( ! $status ) {
			...
			$statuses = array_merge( $statuses, VI_WOOCOMMERCE_ORDERS_TRACKING_TRACKINGMORE::map_statuses(), VI_WOOCOMMERCE_ORDERS_TRACKING_EASYPOST::map_statuses(), VI_WOOCOMMERCE_ORDERS_TRACKING_AFTERSHIP::map_statuses(), VI_WOOCOMMERCE_ORDERS_TRACKING_17TRACK::map_statuses() );
	...
	public static function search_order_item_by_tracking_number( $tracking_number = '', $order_id = '', $email = '', $carrier_slug = '', $unique = true ) {
		...
		if ( $order_id && ! wc_get_order( $order_id ) ) {
			/*If order_is is not an order, search in _order_number meta*/
		...
		if ( $matched_ids ) {
			foreach ( $matched_ids as $id ) {
				$_wot_tracking_carrier = get_post_meta( $id, '_wot_tracking_carrier', true );
				$_wot_tracking_number  = get_post_meta( $id, '_wot_tracking_number', true );
				if ( $_wot_tracking_carrier && $_wot_tracking_number && ( ! $tracking_number || $tracking_number == $_wot_tracking_number ) && ( ! $carrier_slug || $carrier_slug === $_wot_tracking_carrier ) ) {
					$return[] = array(
						'order_id'                     => $id,
						'order_item_id'                => '',
						'order_item_name'              => '',
						...
						foreach ( $results as $result ) {
							if ( $result['meta_value'] ) {
								...
								if ( $result['meta_key'] === '_vi_wot_order_item_tracking_data' ) {
									...
									if ( $current_tracking_data['tracking_number'] == $tracking_number && $current_tracking_data['carrier_slug'] === $carrier_slug && ! in_array( $found_tracking, $found_trackings ) ) {
						...
						foreach ( $results as $result ) {
							if ( $result['meta_value'] ) {
								...
								if ( $result['meta_key'] === '_vi_wot_order_item_tracking_data' ) {
									...
									$found_tracking        = "{$current_tracking_data['tracking_number']}|{$current_tracking_data['carrier_slug']}";
									/*test*/
									if ( $current_tracking_data['tracking_number'] == $tracking_number && $current_tracking_data['carrier_slug'] && ! in_array( $found_tracking, $found_trackings ) ) {
...
//										break;
		...
		if ( $instance->get_params( 'service_carrier_enable' ) ) {
			...
			switch ( $service_carrier_type ) {
				case 'trackingmore':
					...
					$track_data       = $trackingMore->create_tracking( $tracking_number, VI_WOOCOMMERCE_ORDERS_TRACKING_TRACKINGMORE::get_carrier_slug_by_name( $carrier_name ), $order_id );
					if ( $track_data['status'] === 'success' ) {
						$status = $track_data['data']['status'];
						if ( ! count( $tracking_from_db ) ) {
							VI_WOOCOMMERCE_ORDERS_TRACKING_TRACKINGMORE_TABLE::insert( $order_id, $tracking_number, $status, $carrier_slug, $carrier_name, VI_WOOCOMMERCE_ORDERS_TRACKING_DATA::get_shipping_country_by_order_id( $order_id ), '', '', '' );
						}
					} else {
						if ( $track_data['code'] === 4016 ) {
							if ( ! count( $tracking_from_db ) ) {
								VI_WOOCOMMERCE_ORDERS_TRACKING_TRACKINGMORE_TABLE::insert( $order_id, $tracking_number, '', $carrier_slug, $carrier_name, VI_WOOCOMMERCE_ORDERS_TRACKING_DATA::get_shipping_country_by_order_id( $order_id ), '', '', '' );
							}
						} else {
				...
				case 'trackingmore':
					...
					if ( is_array( $bulk_tracking_service_orders ) && count( $bulk_tracking_service_orders ) ) {
						...
						foreach ( $bulk_tracking_service_orders as $bulk_tracking_service_order ) {
							...
							$order_ids[]            = $bulk_tracking_service_order['order_id'];
							$search_tracking_number = "SELECT count(*) FROM {$table_search} WHERE order_id = %s AND tracking_number = %s AND carrier_id = %s";
							$search_tracking_number = $wpdb->prepare( $search_tracking_number, $bulk_tracking_service_order['order_id'], $bulk_tracking_service_order['tracking_code'], $bulk_tracking_service_order['carrier_id'] );
							...
							if ( $check_exist_tracking < 1 ) {
								VI_WOOCOMMERCE_ORDERS_TRACKING_TRACKINGMORE_TABLE::insert( $bulk_tracking_service_order['order_id'], $bulk_tracking_service_order['tracking_code'], '', $bulk_tracking_service_order['carrier_id'], $bulk_tracking_service_order['carrier_name'], $bulk_tracking_service_order['shipping_country_code'], '', '', '' );
							}
							$language = get_post_meta( $bulk_tracking_service_order['order_id'], 'wpml_language', true );
							if ( ! $language && function_exists( 'pll_get_post_language' ) ) {
								$language = pll_get_post_language( $bulk_tracking_service_order['order_id'] );
							}
							$data_batch[] = array(
								...
								'order_id'         => $bulk_tracking_service_order['order_id'],
	...
	public static function get_item_tracking_number( $order_id, $item_id = '', $quantity_index = 1, $empty_tracking = false ) {
		...
		$added_to_paypal       = get_post_meta( $order_id, 'vi_wot_paypal_added_tracking_numbers', true );
		...
		if ( $line_item ) {
			...
			$item_id                                  = '';
			$item_name                                = '';
			$current_tracking_data['tracking_number'] = get_post_meta( $order_id, '_wot_tracking_number', true );
			$current_tracking_data['carrier_slug']    = get_post_meta( $order_id, '_wot_tracking_carrier', true );
			$current_tracking_data['shipment_status'] = get_post_meta( $order_id, '_wot_tracking_status', true );
		}
		...
		$tracking_url_show = apply_filters( 'vi_woo_orders_tracking_current_tracking_url_show', $instance->get_url_tracking( $carrier_url, $tracking_number, $carrier_slug, $order->get_shipping_postcode(), false, true, $order_id ), $item_id, $order_id );
	 ...
	 * @throws Exception
	 */
	public static function set_tracking( $order_id, $tracking_number, $carrier_slug, $item_id, $quantity_index = 1, $add_to_service = true, $add_to_paypal = false, $send_email = false, $send_sms = false ) {
		$instance = self::get_instance();
		$order    = wc_get_order( $order_id );
		$result   = array(
			'is_updated'    => false,
		...
		if ( $item_id ) {
			...
			$old_tracking_number  = get_post_meta( $order_id, '_wot_tracking_number', true );
			$old_tracking_carrier = get_post_meta( $order_id, '_wot_tracking_carrier', true );
			if ( ! $old_tracking_carrier && ! $old_tracking_number ) {
				$result['is_new'] = true;
			}
			if ( $tracking_number != $old_tracking_number || $carrier_slug !== $old_tracking_carrier ) {
				$result['is_updated'] = true;
				update_post_meta( $order_id, '_wot_tracking_number', $tracking_number );
				update_post_meta( $order_id, '_wot_tracking_carrier', $carrier_slug );
				update_post_meta( $order_id, '_wot_tracking_status', '' );
		...
		if ( $result['is_updated'] ) {
			...
			/*Send SMS*/
			if ( $send_sms ) {
				VI_WOOCOMMERCE_ORDERS_TRACKING_ADMIN_SMS::send_sms( $result['tracking_data']['tracking_number'], $result['tracking_data']['carrier_display_name'], $result['tracking_data']['carrier_url'], $result['tracking_data']['tracking_url'], $order, $response, $result['is_new'] );
