...
if ( $api_key == '' ) {
	...
	if ( !$unconfirmed ) {
		...
		echo '</tr></thead>';
		echo '<tbody>';
		echo '<tr><td>' . __( 'API Endpoint', 'zero-bs-crm' ) . '</td><td>' . $endpoint_url . '</td></tr>';
		echo '<tr><td>' . __( 'API Key', 'zero-bs-crm' ) . '</td><td>' . $api_key . '</td></tr>';
		echo '<tr><td>' . __( 'API Secret', 'zero-bs-crm' ) . '</td><td>';
		if ( !empty( $new_api_creds ) ) {
			echo $new_api_creds['secret'];
		} else {
			echo __( 'Your API secret is not displayed for security reasons. If you no longer have access to your API secret, please generate a new one.', 'zero-bs-crm' );
		}
		echo '</td></tr>';
