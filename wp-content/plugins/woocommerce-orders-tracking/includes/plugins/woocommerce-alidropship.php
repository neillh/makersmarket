...
//									'carrier_name'    => $display_name,
					...
					if ( $tracking_number != $old_tracking_data['tracking_number'] || $status_switch_to_shipped ) {
						...
						if ( self::$settings->get_params( 'sms_send_after_aliexpress_order_synced' ) ) {
							...
							VI_WOOCOMMERCE_ORDERS_TRACKING_ADMIN_SMS::send_sms( $tracking_number, $display_name, $carrier_url, self::$settings->get_url_tracking( $carrier_url, $tracking_number, $carrier_slug, $order->get_shipping_postcode(), false, false, $order_id ), $order, $response, $add_new_tracking );
						}
					}
