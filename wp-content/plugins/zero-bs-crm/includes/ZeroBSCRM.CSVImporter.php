...
function jpcrm_csvimporter_lite_preflight_checks( $stage ) {
	...
	if ( $stage == 1 ) {
		...
		if (
			( $csv_file_data['type'] !== 'text/csv' && $csv_file_data['type'] !== 'text/plain' )
			|| pathinfo( $csv_file_data['name'], PATHINFO_EXTENSION ) !== 'csv'
		) {
				...
				<div class="zbscrm-csv-map">
					...
					<form method="post" class="zbscrm-csv-map-form">
						...
						<div class="zbscrm-csv-map-ignorefirst">
							...
							<label for="zbscrmcsvimpignorefirst" ><?php _e('Ignore first line of CSV file when running import.<br />(Use this if you have a "header line" in your CSV file.)','zero-bs-crm'); ?></label>
						</div>
						<hr />
			...
			?><div class="zbscrm-csvimport-wrap">
				...
				<div class="zbscrm-confirmimport-csv">
					...
					<form method="post" enctype="multipart/form-data" class="zbscrm-csv-import-form">
						...
						<input type="hidden" id="zbscrmcsvimpf" name="zbscrmcsvimpf" value="<?php echo esc_attr( $file_details['public_name'] ); ?>" />
						<?php wp_nonce_field( 'zbscrm_csv_import', 'zbscrmcsvimportnonce' ); ?>
						<h3>Import <?php echo zeroBSCRM_prettifyLongInts( $file_details['num_lines'] ); ?> Customers</h3>
						<hr />
						...
						foreach ( $file_details['field_map'] as $fieldID => $fieldTarget ) {
							...
							?>
								<input type="hidden" id="zbscrm-csv-fieldmap-<?php echo ($fieldID-1); ?>" name="zbscrm-csv-fieldmap-<?php echo ($fieldID-1); ?>" value="<?php echo esc_attr( $fieldTarget ); ?>" />
			...
			?><div class="zbscrm-csvimport-wrap">
				...
				<div class="zbscrm-final-stage">
					<div class="zbscrm-import-log">
						<div class="zbscrm-import-log-line"><?php _e( 'Loading CSV File...', 'zero-bs-crm' ); ?> <i class="fa fa-check"></i></div>
						<div class="zbscrm-import-log-line"><?php _e( 'Parsing rows...', 'zero-bs-crm' );?> <i class="fa fa-check"></i></div>
						<div class="zbscrm-import-log-line"><?php echo sprintf( __( 'Beginning Import of %s rows...', 'zero-bs-crm' ), zeroBSCRM_prettifyLongInts( $file_details['num_lines'] ) );?></div>
						<?php 
