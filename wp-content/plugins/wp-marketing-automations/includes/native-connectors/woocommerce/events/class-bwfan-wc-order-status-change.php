...
final class BWFAN_WC_Order_Status_Change extends BWFAN_Event {
	...
	public function validate_v2_event_settings( $automation_data ) {
		...
		if ( 'wc-any' === $current_automation_status_from && 'wc-any' === $current_automation_status_to && $this->validate_order( $automation_data ) ) {
			return true;
		}
		...
		/** checking from any to selected status */
		if ( 'wc-any' === $current_automation_status_from && $order_status_to === $current_automation_status_to && $this->validate_order( $automation_data ) ) {
			return true;
		}

		/** checking from status to selected status */
		if ( $order_status_from === $current_automation_status_from && $order_status_to === $current_automation_status_to && $this->validate_order( $automation_data ) ) {
			return true;
		}

		/** checking from status to wc-any status */
		if ( $order_status_from === $current_automation_status_from && 'wc-any' === $current_automation_status_to && $this->validate_order( $automation_data ) ) {
			return true;
		}
	...
	public function capture_v2_data( $automation_data ) {
		...
		$all_statuses                 = wc_get_order_statuses();
		$automation_data['from']      = $all_statuses[ 'wc-' . $this->from_status ];
		$automation_data['from_slug'] = 'wc-' . $this->from_status;
		$automation_data['to']        = $all_statuses[ 'wc-' . $this->to_status ];
		$automation_data['to_slug']   = 'wc-' . $this->to_status;
