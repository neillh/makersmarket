...
class VI_WOOCOMMERCE_ORDERS_TRACKING_ADMIN_TRACK_PER_QUANTITY {
        ...
        <tr class="plugin-update-tr">
            <td colspan="<?php echo $wp_list_table->get_column_count(); ?>" class="plugin-update colspanchange">
                <div class="update-message inline notice notice-error notice-alt">
                    <p><?php echo VI_WOOCOMMERCE_ORDERS_TRACKING_DATA::wp_kses_post( sprintf( __( 'This plugin is deactivated as its feature was integrated to <strong>WooCommerce Orders Tracking</strong> plugin since version 1.1.0, you can safely delete this custom plugin.', 'woocommerce-orders-tracking' ) ) ) ?></p>
                </div>
            </td>
        </tr>
		<?php
	...
	public function tracking_change( $tracking_change, $quantity_index, $tracking_number, $carrier_slug, $item_id, $order_id ) {
		...
		if ( $order ) {
			...
			if ( $item ) {
				...
				if ( 1 < $quantity_index && $quantity_index <= $quantity ) {
		...
		if ( $order ) {
			...
			if ( $quantity > 1 ) {
				...
				for ( $i = 0; $i < $quantity - 1; $i ++ ) {
					...
					if ( apply_filters( 'vi_woo_orders_tracking_show_tracking_of_order_item_by_quantity', ( ( $manage_tracking !== 'order_only' && $track_per_quantity ) || $tracking_number || $carrier_slug ), $item_id, $order_id ) ) {
                        ...
                        <div class="<?php echo esc_attr( self::set( 'container' ) ) ?>">
                            <div class="<?php echo esc_attr( self::set( 'item-details' ) ) ?>">
                                ...
                                <span class="dashicons dashicons-edit <?php echo esc_attr( self::set( 'button-edit' ) ) ?>"
                                      ...
                                      data-quantity_index="<?php echo esc_attr( $i + 2 ) ?>"
