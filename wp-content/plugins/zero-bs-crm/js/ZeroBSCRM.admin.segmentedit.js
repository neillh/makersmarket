...
function zeroBSCRMJS_segment_bindPostRender(){
	...
	jQuery( '.jpcrm-expand-info' ).off('click').on( 'click', function(){
	...
	jQuery( '.jpcrm-hide-info' ).off('click').on( 'click', function(){
...
function zeroBSCRMJS_segment_buildConditionLine(rule){
	...
	if (typeof rule != "undefined" && rule !== false){
		...
		if (typeof rule.valueconv != "undefined") html += ' data-orig-value="' + jpcrm.esc_attr(rule.valueconv) + '"';
		if (typeof rule.value2conv != "undefined") html += ' data-orig-value2="' + jpcrm.esc_attr(rule.value2conv) + '"';
	...
	html += '<i class="caret up icon jpcrm-hide-info hidden"></i><i class="info circle icon jpcrm-expand-info"></i>';
	...
	// if rule == false it's an empty new one
		...
		if (typeof window.jpcrm_available_conditions != "undefined"){
			...
			if ( rule !== false && typeof window.jpcrm_available_conditions != "undefined" && typeof window.jpcrm_available_conditions[ rule.type ] == "undefined" ){
				...
				// each category
				jQuery.each( window.jpcrm_available_conditions_by_category, function( category_key, category ){

					if ( typeof category.conditions != "undefined" && Object.keys(category.conditions).length > 0 ){
						...
						jQuery.each( category.conditions, function( condition_key, condition ){

							html += '<option value="' + jpcrm.esc_attr( condition.fieldname ) + '"';
							if (typeof rule != "undefined" && rule !== false && typeof rule.type != "undefined" && rule.type == condition.fieldname) html += ' selected="selected"';
	...
	if (typeof selected != "undefined"){
		...
		if (typeof window.jpcrm_available_conditions[selected] != "undefined"){
			...
			if ( 
				...
				jQuery.each(window.jpcrm_available_conditions[selected].operators,function(ind2,ele2){
				...
				if ( window.jpcrm_available_conditions[selected].operators[0] == "tag" || window.jpcrm_available_conditions[selected].operators[0] == "tag_transaction" || window.jpcrm_available_conditions[selected].operators[0] == "extsource" ) {
				
					html += '<input type="hidden" class="zbs-segment-edit-var-condition-operator" value="' + window.jpcrm_available_conditions[selected].operators[0] + '" />';
...
function zeroBSCRMJS_segment_buildConditionCascades2(){
	...
	jQuery('.zbs-segment-edit-condition.dirty').each(function(ind,ele){
		...
		if (typeof selected != "undefined"){
			...
			if (  window.jpcrm_available_conditions[typeselected] && window.jpcrm_available_conditions[typeselected].inputmask ){

				// e.g. jpcrm-inputmask-int
				inputmask_class = ' jpcrm-inputmask-' + window.jpcrm_available_conditions[typeselected].inputmask;
			...
			switch (selected){
				...
				case 'notequal':
					...
					if (typeselected == 'status'){
						...
						// other input
						html += '<input type="text" class="zbs-segment-edit-var-condition-value' + inputmask_class + '" value="'+jpcrm.esc_attr(v)+'" />';
				...
				case 'previousdays':
					...
					html += '<input type="text" class="zbs-segment-edit-var-condition-value jpcrm-inputmask-int" value="'+jpcrm.esc_attr(v)+'" />';
				...
				case 'after':
					...
					html += '<input type="text" class="zbs-date-time zbs-segment-edit-var-condition-value" value="'+jpcrm.esc_attr(v)+'" />';
				...
				case 'afterequal':
					...
					html += '<input type="text" class="zbs-date zbs-segment-edit-var-condition-value" value="'+jpcrm.esc_attr(v)+'" />';
				...
				case 'daterange':
					...
					html += '<input type="text" class="zbs-date-range-condition zbs-segment-edit-var-condition-value" value="' + date_value + '" />';
				...
				case 'datetimerange':
					...
					html += '<input type="text" class="zbs-datetime-range-condition zbs-segment-edit-var-condition-value" value="' + date_value + '" />';
				...
				case 'floatrange':
					...
					html += '<input type="text" class="zbs-float zbs-segment-edit-var-condition-value zbs-segment-pair-input jpcrm-inputmask-float' + inputmask_class + '" value="'+jpcrm.esc_attr(v)+'" placeholder="' + zeroBSCRMJS_segmentLang('eg') + ' 0.00" />';
					html += '<span>' + zeroBSCRMJS_segmentLang('to') + '</span><input type="text" class="zbs-float zbs-segment-edit-var-condition-value-2 zbs-segment-pair-input jpcrm-inputmask-float" value="'+jpcrm.esc_attr(v2)+'" placeholder="' + zeroBSCRMJS_segmentLang('eg') + ' 100.00" />';
				...
				case 'intrange':
					...
					html += '<input type="text" class="zbs-int zbs-segment-edit-var-condition-value zbs-segment-pair-input jpcrm-inputmask-int' + inputmask_class + '" value="'+jpcrm.esc_attr(v)+'" placeholder="' + zeroBSCRMJS_segmentLang('eg') + ' 0" />';
					html += '<span>' + zeroBSCRMJS_segmentLang('to') + '</span><input type="text" class="zbs-int zbs-segment-edit-var-condition-value-2 zbs-segment-pair-input jpcrm-inputmask-int" value="'+jpcrm.esc_attr(v2)+'" placeholder="' + zeroBSCRMJS_segmentLang('eg') + ' 100" />';
...
function zeroBSCRMJS_segment_bindDateRangePicker(){
		...
		jQuery('.zbs-datetime-range-condition').daterangepicker({
								...
								html += '<table class="ui celled striped table"><thead><tr><th colspan="2">' + zeroBSCRMJS_segmentLang('previewTitle') + ':</th></tr></thead><tbody>';
									...
									jQuery.each(previewList.list,function(ind,ele){
										...
										if ( window.jpcrm_contact_stem_URL && ele.id ){
											...
											fn = '<a href="' + jpcrm.esc_attr(window.jpcrm_contact_stem_URL + ele.id) + '" target="_blank">' + jpcrm.esc_html(fn) + '</a>';
...
function jpcrm_js_show_condition_info( ele ){
	...
	// load description
	var condition_key = jQuery( '.zbs-segment-edit-var-condition-type', jQuery(ele).parent() ).find('option:selected').val();
	
	// load description
	var description = zeroBSCRMJS_segmentLang( 'default_description' );
	if ( typeof window.jpcrm_available_conditions[condition_key] != "undefined" && typeof window.jpcrm_available_conditions[condition_key].description != "undefined" && window.jpcrm_available_conditions[condition_key].description != "" ){
	...
	jQuery('.jpcrm-condition-info', jQuery(ele).parent()).text(description).removeClass( 'hidden' );
...
function zeroBSCRMJS_segmentLang( key, fallback ){
	...
	// fallback to globals
	if ( typeof window.zbs_root.lang[key] != "undefined" ) return window.zbs_root.lang[key];
