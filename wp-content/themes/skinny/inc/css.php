<?php
/**
 * Custom class for generating CSS.
 *
 * @package Skinny
 */
...
class CSS {
	...
	public function add( $data ) {
		 ...
		 * @param array    $data    The selectors and properties to add to the CSS.
		 */
		$data  = apply_filters( 'skinny_css_add', $data );
		...
		} else { // Yes, matching selector set, merge declarations.
	...
	private function parse_declarations( $declarations, $tab = '' ) {
		 ...
		 * routines are implemented when the developer calls `skinny_get_css->add()`. Because every property value has
		...
		foreach ( $declarations as $property => $value ) {
			 ...
			 */
			$output .= apply_filters( 'skinny_css_parse_declaration', $parsed_value, $property, $value, $t, $n );
		}
		 ...
		 */
		return apply_filters( 'skinny_css_parse_declarations', $output, $declarations, $tab );
	}
}
